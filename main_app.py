...
class MainWindow(QMainWindow):
    ...
    def add_scroll_option(self):
        ...
        textbox.focusInEvent  = lambda event, sender=textbox : self.text_focus(event, sender)
        textbox.focusOutEvent = lambda event, sender=textbox : self.text_focus(event, sender)
        textbox.setContentsMargins(0,0,20,0)
        ...
        btn.setStyleSheet("QPushButton:hover {background-color: rgb(100,100,100);}" 
    ...
    def text_focus(self, event, sender):
        ...
        if event.gotFocus():    # Focus in.
            ...
            QLineEdit.focusInEvent(sender, event) # Running the rest of the original function.
        else:                   # Focuss out.              
            self.label_to_modify = None
            QLineEdit.focusOutEvent(sender, event) # Running the rest of the original function.
    ...
    def toggle_record(self, button):
        ...
        if self.movie_thread.mode == "Play" and self.movie_thread.ThreadActive and self.mode == 0:
            ...
            if not len(options):
                ...
                msg.setText("You need to add some points to measure before pressing record!")
...
class MovieThread(QThread, face_detector):
    ...
    def get_mouse_pos(self, selected, shape):
        if not self.ThreadActive or self.mode != "Landmarks" or self.image is None: return
...
class PlotWindow(QWidget):
    ...
    def update_plot(self, measures):
        if not measures or len(measures) != len(self.lines): return
